...
public class CustomViewBehind extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// 这里做了修改
		final int contentWidthLeft = getChildMeasureSpec(widthMeasureSpec, 0, width - mWidthOffset);
		final int contentHeight = getChildMeasureSpec(heightMeasureSpec, 0, height);
		mContent.measure(contentWidthLeft, contentHeight);
		if (mSecondaryContent != null){
			final int contentWidthRight = getChildMeasureSpec(widthMeasureSpec, 0, width - mRightWidthOffset);
			mSecondaryContent.measure(contentWidthRight, contentHeight);
		}
	}
	...
	public void scrollBehindTo(View content, int x, int y) {
		...
		if (mMode == SlidingMenu.LEFT) {
			...
			if (x <= content.getLeft())
				vis = View.INVISIBLE;
			scrollTo((int) (getRightBehindWidth() - getWidth() + (x - getRightBehindWidth()) * mScrollScale), y);
	...
	public void drawFade(View content, Canvas canvas, float openPercent) {
		...
		if (mMode == SlidingMenu.LEFT) {
			...
			left = content.getLeft() - getBehindWidth();// 替换原先的getBehindWidth();
			right = content.getLeft();
