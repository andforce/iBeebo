...
public class SendWeiboService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        GeoBean geoBean = intent.getParcelableExtra("geo");

        StatusDraftBean statusDraftBean = intent.getParcelableExtra("draft");

        WeiboSendTask task = new WeiboSendTask(lastNotificationId, token, account, picPath, content, geoBean,
                statusDraftBean);
    ...
    private class WeiboSendTask extends MyAsyncTask<Void, Long, Void> {
        ...
        protected void onPreExecute() {
            ...
            Notification.Builder builder = new Notification.Builder(SendWeiboService.this)
                    ...
                    .setContentTitle(getString(R.string.sending)).setContentText(content).setOnlyAlertOnce(true)
                    .setOngoing(true)
            ...
            if (Utility.isJB()) {
                ...
                IntentFilter intentFilter = new IntentFilter("org.zarroboogs.weibo.SendWeiboService.stop."
                        + String.valueOf(notificationId));

                registerReceiver(receiver, intentFilter);

                Intent broadcastIntent = new Intent("org.zarroboogs.weibo.SendWeiboService.stop."
                        + String.valueOf(notificationId));

                pendingIntent = PendingIntent.getBroadcast(SendWeiboService.this, 1, broadcastIntent,
        ...
        private boolean sendPic(String uploadPicPath) throws WeiboException {
            return new StatusNewMsgDao(token).setPic(uploadPicPath).setGeoBean(geoBean)
                    .sendNewMsg(content, new FileUploaderHttpHelper.ProgressListener() {
            ...
            if (values.length > 0) {
                ...
                if (data != -1) {
                    ...
                    Notification.Builder builder = new Notification.Builder(SendWeiboService.this)
                            ...
                            .setContentTitle(getString(R.string.background_sending)).setNumber((int) (r * 100))
                            .setContentText(content)
                            .setProgress((int) size, (int) data, false).setOnlyAlertOnce(true).setOngoing(true)
                            .setSmallIcon(R.drawable.upload_white);
                    ...
                    Notification.Builder builder = new Notification.Builder(SendWeiboService.this)
                            .setTicker(getString(R.string.send_photo))
                            .setContentTitle(getString(R.string.wait_server_response)).setContentText(content)
        ...
        private void showFailedNotification(final WeiboSendTask task) {
            ...
            PendingIntent pendingIntent = PendingIntent.getActivity(SendWeiboService.this, 0, notifyIntent,
                    PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            if (Utility.isJB()) {
                ...
                PendingIntent retrySendIntent = PendingIntent.getService(SendWeiboService.this, 0, intent,
                        PendingIntent.FLAG_UPDATE_CURRENT);
